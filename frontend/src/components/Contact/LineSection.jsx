...
import LineTitleCard from "./components/LineTitleCard";

const componentMap = {
  Title: LineTitleCard,
};

const dropdownOptions = ["Title"];

const LineSection = () => {
  const [contents, setContents] = useState(() => {
    const saved = localStorage.getItem("line_section_contents");
  ...
  useEffect(() => {
    localStorage.setItem("line_section_contents", JSON.stringify(contents));
  }, [contents]);
        ...
        <>
          {contents.length === 0 ? (
            <div className="bg-white dark:bg-[var(--color-card-secondary)] border border-gray-200 dark:border-gray-700 rounded-lg p-6 text-sm text-center text-gray-500 dark:text-black">
              Belum ada konten LINE. Klik tombol ➕ di atas.
            </div>
          ) : (

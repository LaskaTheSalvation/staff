...
const BannerPictureCard = ({ id, onRemove }) => {
  ...
  const handleFileChange = (rowId, e) => {
    // ✨ LOGIKA TAMBAHAN: Handle jika user batal memilih file
  ...
  const saveRow = (rowId) => {
    ...
    if (!currentRow.nama.trim()) {
      alert("Nama gambar tidak boleh kosong.");
      return;
    }
    ...
    /*
    if (!currentRow.file) {
      alert("Anda harus memilih file gambar sebelum menyimpan.");
      return;
    }
    */
        ...
        <table className="w-full text-sm border-collapse mb-3">
          <thead>
            <tr className="border-b border-gray-300 dark:border-gray-600">
              ...
              <th className="p-2 w-28 text-center text-[var(--color-button-blue)] font-semibold">
                Action
              </th>
            </tr>
          </thead>

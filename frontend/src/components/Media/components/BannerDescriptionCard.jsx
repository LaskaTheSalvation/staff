...
const BannerDescriptionCard = ({ id, onRemove }) => {
  ...
  const addRow = () => {
    // LOGIKA BARU: Mencegah tambah baris jika ada yang belum disimpan
  ...
  const saveRow = (rowId) => {
    // LOGIKA BARU: Validasi input tidak boleh kosong
    const currentRow = rows.find(row => row.id === rowId);
    if (!currentRow.nama.trim() || !currentRow.title.trim()) {
      setError("Nama dan Title wajib diisi.");
      return; // Batalkan penyimpanan jika validasi gagal
    }
  ...
  const deleteRow = (rowId) => {
    // LOGIKA BARU: Mencegah penghapusan baris terakhir
    if (rows.length <= 1) {
        ...
        {error && (
          <Stack sx={{ width: '100%', marginBottom: '1rem' }} spacing={2}>
            <Alert
              ...
              // onClose={() => setError("")} // DIHAPUS: Properti onClose untuk menghilangkan tombol 'x'
              sx={{
...
export default BannerDescriptionCard;
...
import OurServicesTitleCard from "./components/OurServicesTitleCard";
import OurServicesDescriptionCard from "./components/OurServicesDescriptionCard";
import OurServicesServiceCard from "./components/OurServicesServiceCard";

const componentMap = {
  Title: OurServicesTitleCard,
  Description: OurServicesDescriptionCard,
  Card: OurServicesServiceCard,
};

const dropdownOptions = ["Title", "Description", "Card"];

const OurServicesSection = () => {
  const [contents, setContents] = useState(() => {
    const saved = localStorage.getItem("our_services_contents");
  ...
  useEffect(() => {
    localStorage.setItem("our_services_contents", JSON.stringify(contents));
  }, [contents]);
        ...
        <>
          {contents.length === 0 ? (
            <div className="bg-white dark:bg-[var(--color-card-secondary)] border border-gray-200 dark:border-gray-700 rounded-lg p-6 text-sm text-center text-gray-500 dark:text-black">
              Belum ada konten OUR SERVICES. Klik tombol ➕ di atas.
            </div>
          ) : (
            <div className="space-y-4">
              {contents.map((content, idx) => {
                ...
                // Card 1/Card 2 penomoran otomatis
                return (
                  <Component
                    key={content.id}
                    id={content.id}
                    ...
                    cardIndex={
                      content.type === "Card" ? (idx + 1 - contents.filter(c => ["Title", "Description"].includes(c.type)).length) : undefined
                    }
                  />
                );
              })}

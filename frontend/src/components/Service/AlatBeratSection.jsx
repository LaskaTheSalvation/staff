...
import AlatBeratTitleCard from "./components/AlatBeratTitleCard";
import AlatBeratDescriptionCard from "./components/AlatBeratDescriptionCard";
import AlatBeratCard from "./components/AlatBeratCard";

const componentMap = {
  Title: AlatBeratTitleCard,
  Description: AlatBeratDescriptionCard,
  Card: AlatBeratCard,
};

const dropdownOptions = ["Title", "Description", "Card"];

const AlatBeratSection = () => {
  const [contents, setContents] = useState(() => {
    const saved = localStorage.getItem("alat_berat_contents");
  ...
  useEffect(() => {
    localStorage.setItem("alat_berat_contents", JSON.stringify(contents));
  }, [contents]);
        ...
        <>
          {contents.length === 0 ? (
            <div className="bg-white dark:bg-[var(--color-card-secondary)] border border-gray-200 dark:border-gray-700 rounded-lg p-6 text-sm text-center text-gray-500 dark:text-black">
              Belum ada konten Alat Berat. Klik tombol ➕ di atas.
            </div>
          ) : (
              ...
              {contents.map((content, idx) => {
                ...
                return (
                  <Component
                    ...
                    cardIndex={
                      content.type === "Card"
                        ? (idx + 1 - contents.filter(c => ["Title", "Description"].includes(c.type)).length)
                        : undefined
                    }
                  />
                );
...
export default AlatBeratSection;
